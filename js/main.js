...
function(){
  $('.header_wrap').removeClass('active');
    ...
    var swiper = new Swiper(".mySwiper", {
      ...
      //menu_btn버튼을 클릭하면 .mo_menu 나옴
      $('.menu_btn').click(function(){
        $('.mo_menu').animate({'right':0});
        return false;
      });
      //fa-arrow-right 버튼을 클리하면 .mo_menu 들어감
      $('.fa-arrow-right').click(function(){
        $('.mo_menu').animate({'right':'-100vw'});
        return false;
      });
      //태블릿, 모바일 버전 내비게이션(세로 아코디언 메뉴)
      $('.mo_menu nav > ul > li > a').click(function(){
        //만약 클릭한 메뉴에 active가 설정되어 있지 않다면, !=(비교연산자)
        if($(this).attr('class') != 'active'){
          //모든 서브메뉴 들어감
          $('.mo_menu nav .sub').slideUp();
          //모든 메뉴에서 active 제거
          $('.mo_menu nav > ul > li > a').removeClass('active');
